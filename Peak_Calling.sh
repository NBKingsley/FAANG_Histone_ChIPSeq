...
do echo "$file"
    ...
    if [ ! -f Peak_Calls/"$root"_Peaks.bed ]; then
        ...
        macs2 callpeak -t BWA_Output/Narrow/$file -c BWA_Output/Duplicates/Input_"$tissue".duplicate-removed.bam -f BAM -n Peak_Calls/$root -g $genome -q 0.01 -B --SPMR --keep-dup all --fix-bimodal --extsize 200
...
do echo "$file"
    ...
    if [ ! -f Peak_Calls/"$root"_Peaks.bed ]; then
        echo "$file"
        echo 'Macs2 peak-calling'
        macs2 callpeak -t BWA_Output/Medium/$file -c BWA_Output/Duplicates/Input_"$tissue".duplicate-removed.bam -f BAM -n Peak_Calls/$root -g $genome -q 0.05 -B --SPMR --keep-dup all --fix-bimodal --extsize 200
...
do echo "$file"
    ...
    if [ ! -f Peak_Calls/"$root"_Peaks.bed ]; then
        if [ $filter = 'loose' ]; then echo $filter
            ...
            echo "$tissue"
            echo 'Macs2 peak-calling'
            macs2 callpeak -t BWA_Output/Broad/$file -c BWA_Output/Duplicates/Input_"$tissue".loose.duplicate-removed.bam -f BAM -n Peak_Calls/$root -g $genome -q 0.05 --broad --broad-cutoff 0.1 -B --SPMR --keep-dup all --fix-bimodal --extsize 200
            ...
            echo "$file"
            echo 'Macs2 peak-calling'
            macs2 callpeak -t BWA_Output/Broad/$file -c BWA_Output/Duplicates/Input_"$tissue".duplicate-removed.bam -f BAM -n Peak_Calls/$root -g $genome -q 0.05 --broad --broad-cutoff 0.1 -B --SPMR --keep-dup all --fix-bimodal --extsize 200
